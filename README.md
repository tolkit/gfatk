...
`gfatk resolve` additionally requires a GAF file of long reads aligned back to the assembly graph (e.g. from <a href="https://github.com/maickrau/GraphAligner">`GraphAligner`</a>), and optionally a gene annotation to weight gene-bearing segments during resolution -- either a GFF (`--gff`, hits filtered to `coverage=full`) or, directly, an oatk-style BED file (`--bed`, columns `seq_name align_from align_to gene_name score_capped_at_1000 strand`, `seq_name` referencing the GFA's own segment IDs, no conversion step needed). Every BED row counts as a gene call regardless of score -- oatk's score isn't a clean complete/partial signal the way GFF's `coverage` attribute is, so filtering on it risks dropping real single-copy genes. Both `--gff` and `--bed` can be given together; the resulting gene-bearing segments are combined.
